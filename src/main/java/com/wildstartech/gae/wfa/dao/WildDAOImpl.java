...
import com.wildstartech.wfa.dao.WildDAOResources;
 ...
 * <p>
 * This <code>Data Access Object (DAO)</code> provides users with the ability to
 * return partial entities as opposed to fully reading all properties of an
 * entity when objects are retrieved from the datastore. The <code>DAO</code>
 * will allow users to add a list of <code>projectionFields</code> prior to
 * performing any query. Once the query is executed and the results are
 * returned, the list of <code>projectionFields</code> will be cleared unless
 * the <code>retainProjectionFields<code> property is set equal to 
 * <code>true</code>.
 * </p>
 * 
...
implements WildDAO<T, W> {
   ...
   private static String PROPFILE_NAME="wild-dao-admin.properties";
   ...
   /**
    * Return a localized message identified by the specified {@code resourceId}
    * customized with the values contained in the specified in the
    * {@code Object} array.
    */
   public static String localizeMessage(String resourceId, Object[] params) {
      logger.entering(_CLASS, "localizeMessage(String,Object[])", new Object[] { resourceId, params });
      String msg = "";

      msg = Localization.getString(RESOURCE_BUNDLE, resourceId, params);
   ...
   private static void loadProperties() {
      ...
      in = cl.getResourceAsStream(PROPFILE_NAME);
      try {
         if (in != null) {
            props.load(in);
            // ***** password
            ...
            if (password == null) {
               msg = WildDAOImpl.localizeMessage(
                     WildDAOResources.MSGKEY_ADMIN_PWD_NULL, null);
               logger.warning(msg);
            ...
            msg = WildDAOImpl.localizeMessage(
                  WildDAOResources.MSGKEY_PROPERTIES_NOT_FOUND,
                  new Object[] {PROPFILE_NAME});
            logger.log(Level.SEVERE, msg);
         } // END if (in != null)
      } catch (IOException ex) {
         msg = WildDAOImpl.localizeMessage(
               WildDAOResources.MSGKEY_PROPERTIES_ERROR_READING, 
               null);
   ...
   public String getResourceBundleBaseName() {
      ...
      logger.exiting(_CLASS, "getResourceBundleBaseName()",
            WildDAO.RESOURCE_BUNDLE);
      return WildDAO.RESOURCE_BUNDLE;
   }
    ...
    */
   /*
    * private void loadRules() { logger.entering(_CLASS,"loadRules()");
    * ClassLoader cl=null; InputStream in=null; Properties props=null; String
    * className=null;
    * 
    * cl=WildDAOImpl.class.getClassLoader();
    * in=cl.getResourceAsStream(className); if (logger.isLoggable(Level.FINEST))
    * { logger.finest("Attempting to read DAO Configuration File"); } if (in !=
    * null) { props=new Properties(); try { props.load(in); } catch (IOException
    * ex) { logger.log(Level.SEVERE, "Error loading DAO properties file.", ex);
    * } // END try/catch } else {
    * logger.warning("Unable to locate "+className+".properties file."); } //
    * END if (in != null) logger.exiting(_CLASS,"loadRules()"); }
    */
   /**
   ...
   protected boolean isUserValid(PersistentUser user) {
      ...
      if (user != null) {
         ...
         msg=getLocalizedMessage(
               WildDAOResources.MSGKEY_PARAM_NULL, 
               new Object[] {"user"});
         logger.warning(msg);
      } // END if (user != null)
    ...
    * 
    * @param fieldName
    *           The property to be added to the list of projection fields to be
    *           returned by the next query executed by the data access object.
   ...
   public boolean addProjectionField(Property property) {
      ...
      if (property != null) {
         /*
          * The property is not null, so let's check the internal
          * projectionFields list to see if it is null.
          */
         ...
         if (!this.projectionFields.contains(property)) {
            /*
             * Only add the item if it does not already exist - properties can
             * only be added once.
             */
    ...
    * <p>
    * The <code>projectionFields</code> stored in the data access object are
    * replaced with the contents of the list passed as a parameter.
    * </p>
    * 
    * @param fields
    *           The properties to use in the projection query.
    */
   ...
   public W delete(T object, UserContext ctx) throws DAOException {
      logger.entering(_CLASS, "delete(T,UserContext<Key>)", 
      ...
      if (object != null) {
         if (object instanceof WildObjectImpl) {
            ...
            if ((identifier != null) && (identifier.length() > 0)) {
               ...
               msg=getLocalizedMessage(
                     WildDAOResources.MSGKEY_DELETE_FAIL_NOT_SAVED,
                     null);
               logger.warning(msg);
         ...
         msg=getLocalizedMessage(
               WildDAOResources.MSGKEY_PARAM_DELETE_NULL,null);
         logger.warning(msg);
   ...
   public W save(T object, UserContext ctx) throws DAOException {
      logger.entering(_CLASS, "save(T,UserContext<Key>)", 
            new Object[] { object, ctx });
      String msg=null;
      W savedObject = null;
      ...
      if (object != null) {
         try {
            ...
            /*
             * No-txn if (txn.isActive()) { txn.rollback();
             * logger.severe("Transaction rolling back."); } // END if
             * (txn.isActive())
             */
   ...
   @SuppressWarnings("unchecked")
   public W save(T object, UserContext ctx, Transaction txn) throws DAOException {
      logger.entering(_CLASS, "save(W,UserContext,Transaction)", 
            new Object[] { object, ctx, txn });
      ...
      if (object != null) {
         ...
         if (newObject) {
            ((WildObjectImpl<?>) wildObject).setIdentifier(String.valueOf(key.getId()));
         } // END if (newObject)

         /*
          * Have the wild object re-populate itself from the entity. At a
          * minimum this will cause the create/modification fields to be
          * re-populated.
          */
   ...
   public List<W> findAll(UserContext ctx) throws DAOException {
      ...
      if ((ctx != null) && (ctx.isAuthenticated())) {
         ...
         if (ctx == null) {
            ...
            msg = getLocalizedMessage(
                  WildDAOResources.MSGKEY_NOT_AUTHENTICATED,
   ...
   protected Entity findEntityByIdentifier(String identifier, UserContext ctx) 
      ...
      if (identifier != null) {
         ...
         if (memcache.contains(key)) {
            ...
            try {
               ...
               msg=getLocalizedMessage(
                     WildDAOResources.MSGKEY_NOT_FOUND, 
      ...
      logger.exiting(_CLASS, "findEntityByIdentifier(String,UserContext)", entity);
      return entity;
   }

   public W findByIdentifier(String identifier, UserContext ctx) throws DAOException {
      logger.entering(_CLASS, "findByIdentifier(String,UserContext)", new Object[] { identifier, ctx });
      Entity entity = null;
      String msg = null;
      ...
      if ((identifier != null) && (ctx != null) && (ctx.isAuthenticated())) {
         ...
         logger.severe(msg);
      } // END if ((identifier != null) && (ctx !=
        // null)&&(ctx.isAuthenticated()))
    ...
    * 
    * <p>
    * The <code>Query</code> object is a Google App Engine specific entity that
    * is used to search for data in the <code>DataStore</code>. The
    ...
    * in managing the amount of data returned as part of a given query.
    * </p>
    * <h1>Variable Arguments</h1>
    * 
    * 
    * @param query
    *           The query to be used to search for the specified objects.
    * @param ctx
    *           The credentials that should be used to access persistent data.
    * @param args
    *           Variable set of optional arguments that will be used to manage
   ...
   protected List<W> _findByQuery(QueryWrapper query, UserContext ctx) {
      ...
      if ((query != null) && (ctx != null)) {
         ...
         if (ctx.isAuthenticated()) {
            ...
            /*
             * Set a flag on the query object to ONLY return keys IF there are
             * NO sort criteria.
             */
            /*
             * if (sortCriteria == null) { gQuery.setKeysOnly(); } // END if
             * (sortCriteria == null)
             */
               ...
               if (property != null) {
                  ...
                  if ((propertyName != null) && (propertyName.length() > 0)) {
                     switch (criterion.getSortOrder()) {
                        case DESCENDING:
                           sortDirection = com.google.appengine.api.datastore.Query.SortDirection.DESCENDING;
                           break;
                        default:
                           sortDirection = com.google.appengine.api.datastore.Query.SortDirection.ASCENDING;
                           break;
                     } // END switch (criterion.getSortOrder())

                     /*
                      * The property is NOT null, so let's add the sort in the
                  ...
                  } // END if ((propertyName != null) && (propertyName.length()
                    // > 0))
               } // END
            ...
            if ((this.projectionFields != null) && (this.projectionFields.size() > 0)) {
               for (Property prop : this.projectionFields) {
                  gQuery.addProjection(new PropertyProjection(prop.getName(), prop.getType()));
            ...
            /*
             * Fetch the results as a List<Entity> Specifying the page size and
             * offset to use when retrieving the data.
             */
         ...
         if (query == null)
            logger.warning("The Query parameter is null.");
         if (ctx == null)
   ...
   public List<W> findByQuery(Query query, UserContext ctx) {
      logger.entering(_CLASS, "findByQuery(Query,UserContext)", new Object[] { query, ctx });
      List<W> list = null;
