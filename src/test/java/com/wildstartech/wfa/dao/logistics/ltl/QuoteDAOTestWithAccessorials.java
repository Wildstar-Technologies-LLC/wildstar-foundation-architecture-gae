...
public class QuoteDAOTestWithAccessorials extends WildDAOTest {
  ...
  public void testAccessorials() {
    ...
    charge=dao.create();
    charge.setAmount(this.accessorial1Amount);
    try {
      charge.setDescription(this.accessorial1Description);
    } catch (ChargeDescriptionTooLongException ex) {
      // No-Op
    }
    charge.setQuantity(this.accessorial1Quantity);
  ...
  public void testAccessorialSave() {
    ...
    assert charge.getDescription().equalsIgnoreCase(
        this.accessorial1Description);
    testCharge=new BigDecimal(this.accessorial1Amount);
    testCharge.setScale(2,RoundingMode.HALF_UP);
    assert charge.getAmount().compareTo(testCharge) == 0;    
  }
  ...
  public void testCorrectAccessorials() {
    ...
    newCharge.setAmount(this.accessorial2Amount);
    try {
      newCharge.setDescription(this.accessorial2Description);
    } catch (ChargeDescriptionTooLongException ex) {
      // No-Op
    }
    newCharge.setQuantity(this.accessorial2Quantity);
